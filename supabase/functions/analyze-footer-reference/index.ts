...
serve(async (req) => {
  ...
  const ctx = newTrace('analyze-footer-reference', req);
  ...
  try {
    ...
    if (!imageUrl) {
      return jsonResponse(req, { success: false, error: 'imageUrl is required' }, 400);
    }
    ...
    return jsonResponse(req, { success: false, error: sanitizeError(ctx, error) }, 500);
  }
});
