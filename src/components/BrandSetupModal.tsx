...
export const BrandSetupModal = ({
  open,
  scrapedData,
  onComplete,
  onClose,
}: BrandSetupModalProps) => {
  ...
  // Update state when scrapedData changes
  useEffect(() => {
    if (scrapedData) {
      setBrandName(scrapedData.name || '');
      if (scrapedData.primaryColor) setPrimaryColor(scrapedData.primaryColor);
      if (scrapedData.secondaryColor) setSecondaryColor(scrapedData.secondaryColor);
      if (scrapedData.socialLinks) setSocialLinks(scrapedData.socialLinks);
      if (scrapedData.allLinks) setAllLinks(scrapedData.allLinks);
    }
  }, [scrapedData]);
  ...
  const handleSave = async () => {
    if (!darkLogoUrl || !lightLogoUrl || !scrapedData) {
    ...
    setIsSaving(true);
    try {
      const domain = extractDomain(scrapedData.url);
      ...
      const { data, error } = await supabase
        ...
        .insert({
          name: brandName,
          domain,
          website_url: scrapedData.url.startsWith('http') ? scrapedData.url : `https://${scrapedData.url}`,
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Complete Brand Setup</DialogTitle>
          <DialogDescription>
            We detected <strong>{scrapedData?.name || 'a brand'}</strong> from your email. Please upload logos to continue.
          </DialogDescription>
        </DialogHeader>
